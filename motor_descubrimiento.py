...
Usa la interfaz genérica nora_data_ingest (una fila por celda/mes) y separa
...
BASE_TABLE = os.getenv("NORA_DATA_SOURCE", "nora_data_ingest")
...
for a, b in combinations(dynamic, 2):
    for lag in range(MAX_LAG + 1):
        left = df[["cell_id", "observed_at", a]].copy(); right = df[["cell_id", "observed_at", b]].copy()
        ...
        if len(pair) < MIN_POINTS:
            continue
        rs = pair.groupby("cell_id").apply(lambda g: g[a].corr(g[b]) if len(g) >= MIN_POINTS else np.nan).replace([np.inf, -np.inf], np.nan).dropna()
        if len(rs):
            pairs.append({"a": a, "b": b, "lag_months": lag, "mean_within_cell_r": float(rs.mean()), "median_within_cell_r": float(rs.median()), "cells": int(len(rs))})
...
if "ndvi" in dynamic and "precip_mm" in dynamic:
    ...
    for cell_id, g in df.groupby("cell_id", sort=False):
        ...
        for lag in range(MAX_LAG + 1):
            ...
            if pd.notna(r): lag_records.append((cell_id, lag, float(r)))
    ...
    if not sens.empty:
        ...
        if soil_cols:
            ...
            for col in soil_cols:
                if best[col].notna().sum() >= MIN_POINTS:
                    r = best["rain_ndvi_r"].corr(best[col])
                    if pd.notna(r): soil_effects.append({"soil_variable": col, "correlation_with_best_rain_ndvi_sensitivity": float(r), "cells": int(best[["rain_ndvi_r", col]].dropna().shape[0])})

index = pd.MultiIndex.from_frame(df[["cell_id", "observed_at"]]); cube = df.set_index(index)[dynamic]
anomaly_summary = {}
for col in dynamic:
    x = cube[col]; med = x.groupby(level=0).transform("median"); mad = (x - med).abs().groupby(level=0).transform("median"); z = (x - med) / (1.4826 * mad.replace(0, np.nan))
    anomaly_summary[col] = {"mean_abs_z": float(np.nanmean(np.abs(z))) if np.isfinite(z).any() else None, "cells_with_anomalies": int(np.isfinite(z).groupby(level=0).any().sum())}

hypotheses = []
for p in pairs[:30]:
    if abs(p["mean_within_cell_r"]) < 0.35: break

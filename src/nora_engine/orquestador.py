...
class Orquestador:
    ...
    def planificar(self, caso, zona, periodo):
        ...
        variables=[v["nombre"] for v in plan["variables"]]; fuentes=resolver_fuentes(variables)
        tareas=construir_tareas(fuentes,zona,periodo); valid=[validar_tarea(t) for t in tareas]
        self._set("adquisicion",25,{"plan":plan,"fuentes":fuentes,"tareas":tareas,"validaciones":valid}); return self.estado
    def integrar(self, datos):
        ...
        alineados={k:alinear_temporal(v) for k,v in aptos.items()}; self._set("alineacion",55,calidad); return alineados
